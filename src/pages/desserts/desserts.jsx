...
const Desserts = () => {
  const [recipes, setRecipes] = useState([]);
    ...
    axios.get("http://localhost:3000/api/recipes") // Replace with your API endpoint
      .then(response => {
        ...
        setRecipes(response.data.filter(recipe => recipe.recipeType && recipe.recipeType.trim().toLowerCase() === "dessert")); // Assuming the response data is an array of recipes
      })
      .catch(error => {
  ...
  return (
    <div>
        <Header />
        <h1>Mes desserts</h1>
        <p>Découvrez nos recettes de desserts savoureux et faciles à préparer.</p>
        <ul>
        {recipes.map(recipe => (
          <li key={recipe.id}>
